...
import { create3dPage } from '../simple/webpage3d.js';
import { objectScene, cssScene, camera } from '../simple/main.js';
import { initWebcamPage } from '../simple/initWebcamPage.js';
  ...
  // console.log(entity_data.time, entity_data.z, entity_data.type)
    ...
    //     // camera.rotation.set( rx, ry - Math.PI, rz );
        ...
        const page = create3dPage(
            ...
            100, 100,
            0.01,
            new THREE.Vector3(5 * Math.sin(Math.PI * (Object.keys(entities).length - 1)/10), 0, -5 * Math.cos(Math.PI * (Object.keys(entities).length - 1)/10)),
            // new THREE.Vector3(-5 * Math.sin(Math.PI * Object.keys(entities).length / 10), 0, -5 * Math.cos(Math.PI * Object.keys(entities).length) / 10),
